...
|3              |[Сбор данных из открытых источников](collection_data_open_sources/)|Это пример некоторых методов получения данных из Google Trends и записи в собственную БД sqlite3. Настройки сценария автообновления БД. |`pytrends` `sqlite3` `pandas` `datetime` `zoneinfo` `re`|
...
|9              |[А/А-тестирование. Поиск ошибки системы сплитирования данных у мобильного приложения.](aa_test/)|Проверили утверждение о поломке сплит системы и нашли причины. Считали результаты A/A-теста, проверяли FPR на конверсии в покупку. Сделали выводы, на основе анализа результатов A/A-теста.|`pandas` `numpy` `scipy` `random` `tqdm` `matplotlib` `seaborn`|
|10              |[RFM анализ](rfm_analysis/)|Провели RFM анализ. В каждом подсегменте поделили пользователей на 4 класса.|`pandas` `numpy` `seaborn`|
